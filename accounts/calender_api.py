...
def get_calender_events(technician,event_count=1000,timeMin=None,timeMax=None):
    """Shows basic usage of the Google Calendar API.
    Prints the start and name of the next 10 events on the user's calendar.
    """
...
def get_free_times(technician,events):
    """Shows basic usage of the Google Calendar API.
    Prints the start and name of the next 10 events on the user's calendar.
    """
...
def get_current_address_date(events, customer,current_date,time):
    ...
    for event in events:
        ...
        if start.date() == current_date:
            if start.time() < time < end.time() and start.date() == datetime.datetime.now().date():
                if event.get('summary').lower() == 'break':
                    if next_event:
                        time_gap = next_start - end
                        time_gap = time_gap.seconds/(60)
...
def get_previous_event(events, current_date, time):
    ...
    count = -1
    for event in events:
        if event.get('summary').lower() != 'off':
            start = dateutil.parser.parse(event['start'].get('dateTime', event['start'].get('date')))
            end = dateutil.parser.parse(event['end'].get('dateTime', event['end'].get('date')))

            nex_start = dateutil.parser.parse(events[count+1]['start'].get('dateTime', events[count+1]['start'].get('date')))
            nex_end = dateutil.parser.parse(events[count+1]['end'].get('dateTime', events[count+1]['end'].get('date')))

            if start.date() == current_date:
                if end.time() <= time and end.time().hour != 9:
                    is_next = False
                    if time.hour < nex_start.time().hour:
                        is_next = True
                    elif time.hour == nex_start.time().hour:
                        if  nex_start.time().minute > time.minute:
                            is_next = True

                    is_previous = False
                    if time.hour > end.time().hour:
                        is_previous = True
                    elif time.hour == end.time().hour:
                        if end.time().minute < time.minute:
                            is_previous = True

                    if is_previous:

                          # if start.date().day == 9:
                        #     print("end.time()end.time()",event.get("summary"),end.time(),event.get('location'))
    ...
    for event in events:
        if event.get('summary').lower() != 'off':
            ...
            if start.date() == current_date:
                ...
                if start.time() <= time and start.time().hour != 9:
                    is_next = False
                    if time.hour < nex_start.time().hour:
                        is_next = True
                    elif time.hour == nex_start.time().hour:
                    ...
                    is_previous = False
                    if time.hour > start.time().hour:
                        is_previous = True
                    elif time.hour == start.time().hour:
                        if start.time().minute < time.minute:
                            is_previous = True
